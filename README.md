...
- [初始化验证与数据初审](docs/初始化验证与数据初审.md)：8项测试、真实表型计数及Kas-2重复记录。
...
已核验HDF5为1135×10,709,949，真实交集FT10=1003、rosetteDM=407；其中重复ID尚待解决。下一步补齐REF/ALT与原始缺失语义、处理重复记录、构造正式训练输入，再冻结性状与划分。PC-only、LD/RKHS/多核、近亲分块及结构化SSM尚未实现；不以合成自检或理论复杂度宣称真实全密度预测成功。
